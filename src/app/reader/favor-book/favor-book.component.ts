...
export class FavorBookComponent implements OnInit {
  ...
  renderBookList(startNum: number) {
    ...
    if (this.bookList.length == 0) {
      ...
      this.translate.stream('favorBook.emptyMessage').subscribe((res) => {
        p1.innerHTML = res;
      });
      ...
      for (let i = startNum; i < endNum; i++) {
        ...
        this.translate.stream('favorBook.delButton').subscribe((res) => {
          delButton.innerHTML = res;
        });
        delButton.style.padding = '5px';
        delButton.style.marginTop = '5px';
        delButton.addEventListener(
          'click',
          this.delFavorBook.bind(this, this.bookList[i]._id)
        );
  ...
  delFavorBook(bookID) {
    ...
    if (
      window.confirm('Please confirm if you want to remove it from favor list')
    ) {
      this.readerService
        .delFavorBook(readerID, favorBookDto)
        .subscribe((data) => {
          if (data >= 0) {
  ...
  ngOnDestroy() {
    ...
    this.logger.info(
      'Success cleaned all eventListeners added by favor book component'
    );
  }
}
